...
router.get('/', (req, res) => {
  ...
  const total = db.get(`SELECT COUNT(*) as c FROM offers o LEFT JOIN categories c ON o.category_id=c.id LEFT JOIN servers s ON o.server_id=s.id ${ws}`, params);
...
router.get('/:id', (req, res) => {
  ...
  const bids = db.all('SELECT b.*, u.username FROM bids b JOIN users u ON b.user_id=u.id WHERE b.offer_id=? ORDER BY b.amount DESC LIMIT 20', [req.params.id]);
  res.json({ ...offer, bids });
});
...
router.post('/:id/bid', auth, (req, res) => {
  const user = db.get('SELECT is_banned FROM users WHERE id=?', [req.user.id]);
  if (user?.is_banned) return res.status(403).json({ error: 'Konto zablokowane' });
  const { amount } = req.body;
  const offer = db.get("SELECT * FROM offers WHERE id=? AND type='auction' AND status='active'", [req.params.id]);
