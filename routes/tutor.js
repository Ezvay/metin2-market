...
router.post('/request', auth, async (req, res) => {
  ...
  const offer = db.get('SELECT * FROM offers WHERE id=? AND status=?', [offer_id, 'active']);
  ...
  const existing = db.get("SELECT * FROM tutor_requests WHERE offer_id=? AND buyer_id=? AND status='pending'", [offer_id, req.user.id]);
  if (existing) return res.status(409).json({ error: 'Już złożyłeś prośbę o TuTora dla tego ogłoszenia' });

  db.runInsert('INSERT INTO tutor_requests (offer_id,buyer_id,seller_id,payment_method) VALUES (?,?,?,?)',
    [offer_id, req.user.id, offer.seller_id, payment_method]);

  const seller = db.get('SELECT * FROM users WHERE id=?', [offer.seller_id]);
  const buyer = db.get('SELECT username FROM users WHERE id=?', [req.user.id]);
...
reviewsRouter.post('/', auth, (req, res) => {
  const { offer_id, reviewed_id, rating, comment } = req.body;
  if (!reviewed_id || !rating || rating < 1 || rating > 5)
    return res.status(400).json({ error: 'Nieprawidłowe dane' });

  const existing = db.get('SELECT * FROM reviews WHERE reviewer_id=? AND offer_id=?', [req.user.id, offer_id]);
  if (existing) return res.status(409).json({ error: 'Już wystawiłeś opinię dla tej transakcji' });

  db.runInsert('INSERT INTO reviews (offer_id,reviewer_id,reviewed_id,rating,comment) VALUES (?,?,?,?,?)',
    [offer_id || null, req.user.id, reviewed_id, rating, comment || null]);

  // Update user rating
  const stats = db.get('SELECT AVG(rating) as avg, COUNT(*) as cnt FROM reviews WHERE reviewed_id=?', [reviewed_id]);
  db.run('UPDATE users SET rating=?, rating_count=? WHERE id=?', [stats.avg, stats.cnt, reviewed_id]);

  res.json({ message: 'Opinia dodana!' });
});

reviewsRouter.get('/user/:id', (req, res) => {
