...
router.get('/', async (req, res) => {
  res.json(await db.all("SELECT * FROM servers WHERE is_active=1 ORDER BY id ASC"));
});

// GET categories for a server
router.get('/:slug/categories', async (req, res) => {
  const srv = await db.get('SELECT * FROM servers WHERE slug=?', [req.params.slug]);
  if (!srv) return res.status(404).json({ error: 'Serwer nie istnieje' });
  res.json(await db.all('SELECT * FROM categories WHERE server_id=? ORDER BY id', [srv.id]));
});
...
router.post('/', auth, requireRole('admin'), async (req, res) => {
  ...
  try {
    const r = await db.runInsert('INSERT INTO servers (name,slug,description,rates,website) VALUES (?,?,?,?,?)',
...
router.patch('/:id/toggle', auth, requireRole('admin'), async (req, res) => {
  const srv = await db.get('SELECT * FROM servers WHERE id=?', [req.params.id]);
  if (!srv) return res.status(404).json({ error: 'Nie znaleziono' });
  await db.run('UPDATE servers SET is_active=? WHERE id=?', [srv.is_active ? 0 : 1, req.params.id]);
  res.json({ message: srv.is_active ? 'Serwer ukryty' : 'Serwer aktywny' });
});

// ADMIN: add category to server
router.post('/:slug/categories', auth, requireRole('admin'), async (req, res) => {
  const srv = await db.get('SELECT * FROM servers WHERE slug=?', [req.params.slug]);
  if (!srv) return res.status(404).json({ error: 'Serwer nie istnieje' });
  const { name, slug, icon } = req.body;
  if (!name || !slug) return res.status(400).json({ error: 'Nazwa i slug wymagane' });
  try {
    await db.runInsert('INSERT INTO categories (server_id,name,slug,icon) VALUES (?,?,?,?)', [srv.id, name, slug, icon||'📦']);
